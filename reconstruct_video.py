...
def extract_frames(video_path):
    ...
    while ret:
        ...
        frames.append(frame)
    cap.release()
    print(f"[I] Extracted {len(frames)} frames (FPS metadata: {fps})")
    return frames, fps
...
def choose_start_frame(grays):
    ...
    for i in range(n):
        ...
        for j in range(sample_k):
            idx = (i + j + 1) % n
            sims.append(ssim(grays[i], grays[idx], data_range=grays[idx].max() - grays[idx].min()))
        avg_sim.append(np.mean(sims))
    return int(np.argmin(avg_sim))


def reconstruct_sequence(edges, grays, start=None, beam_width=BEAM_WIDTH, max_steps=2000):
    n = len(edges)
    if n == 0:
        return []

    if start is None:
        start = choose_start_frame(grays)
    ...
    while beam and step < max_steps:
        ...
        if not new_beam:
            visited_nodes = beam[0][2]
            remaining = [i for i in range(n) if i not in visited_nodes]
            if not remaining:
                best_seq = beam[0][1]
                break
            candidate_scores = []
            for j in remaining:
                max_s = 0.0
                for i_idx in visited_nodes:
                    for s, nb in edges[i_idx]:
                        if nb == j and s > max_s:
                            max_s = s
            ...
            new_beam = [(beam[0][0], new_seq, visited_nodes | {best_next})]
...
def main(video_path, out_path):
    ...
    seq = reconstruct_sequence(edges, grays) 
